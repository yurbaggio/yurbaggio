...
CprocObj cproc_New() {
	CprocObj self = (CprocObj) calloc(1, sizeof(struct cprocObj));
	return self;
}
...
int cproc_Init(CprocObj self, void (*enInt_var)(void), void (*disInt_var)(void), int (*va_print)(const char *, ...), unsigned int (*p_recv_var)(char *BufferPtr), char * delimiters_var, unsigned int queueSize){
    return cproc_InitMultiTerm(self, enInt_var, disInt_var, va_print, va_print, p_recv_var, delimiters_var, queueSize);
}
...
int cproc_AddEscape(CprocObj self, void (*cmdEscape)(const char * cmdStr)) {
	self->cmdEscape = (CprocCmd *) calloc(1, sizeof(CprocCmd));
...
int cproc_Add(CprocObj self, const char * cmd, void (*cmdInvoke)(const char * cmdStr)) {
	...
	int exist = cproc_CheckCommand(self, cmd);
	...
	self->cmdList[self->cmdCount] = (CprocCmd *) calloc(1, sizeof(CprocCmd));
...
void cproc_Invoke(CprocObj self, const char * str){
	...
	self->va_print_src("command not recognized \n\r\n\r"); 
}
  ...
  int status;
    ...
    while (self->p_recv(squeue_char) != 0){
		...
		if ((squeue_char[0] == '\r') || (self->squeue_msg_index >= CPROC_LINEBUFF_SIZE)){
			...
			cproc_Invoke(self, self->cqueue_msg);
			return;
		}
